...
| `sensor.py` | Two diagnostic `CoordinatorEntity` sensors per lamp: **Brightness** (%, `dlight_{id}_brightness`) and **Color Temperature** (K, `dlight_{id}_color_temp`). Both go unavailable when `coordinator.last_update_success` is `False`. No extra polling — reads from the coordinator's existing state dict. |
| `update.py` | `DLightUpdateEntity`: surfaces `coordinator.info["swVersion"]` as `installed_version` with `latest_version = None` (no OTA source known). Goes unavailable when the coordinator loses the lamp. Enables future OTA support if dLight publishes a firmware manifest. |
...
| `config_flow.py` | UDP discovery → pick list, manual entry, reconfigure step, and a `dhcp` step (manifest matches `registered_devices`) that self-heals a known lamp's IP when DHCP hands it a new one. Discovery uses `discover_devices_stream` for faster first-lamp response. There is **no options flow** (removed per ADR-0010; poll interval is fixed). Known lamps rediscovered on a new IP during a user-initiated scan are also self-healed; manual re-add of a known lamp refreshes its IP too. |
